...
The microphone records while the session is active. After 1.8 seconds of quiet, Whisper transcribes the utterance. The server answers by default, retaining only obvious unfinished endings and explicit requests for thinking time (see `turn_detection.py`). Qwen generates replies but no longer decides whether a turn is complete: its classification incorrectly blocked ordinary questions. This is a limited text heuristic, not semantic or acoustic turn detection; unrecognized incomplete thoughts can still receive a reply. Audio is retained across pauses. Resuming sustained speech cancels the pending browser request and invalidates its response, so an outdated answer cannot overwrite the continuation. **Bitirdim** remains an optional override. While the teacher speaks, tap **Araya gir** to stop playback and resume recording. Microphone capture is suspended during teacher playback to avoid feedback.

Run the turn regression checks with `.talk-agent/.venv/Scripts/python.exe .talk-local/test_turn_detection.py` from the repository root.
