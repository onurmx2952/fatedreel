...
The microphone records while the session is active. A quiet interval only splits the audio into transcription segments. It does not trigger an answer. The user must tap **Bitirdim** or say **bitti** at the end of a segment. Voice recognition can miss the Turkish word; the button is reliable. While the teacher speaks, tap **Araya gir** to stop playback and resume recording. This is explicit turn control, not semantic automatic turn detection. No claim is made that it detects thought pauses naturally.
